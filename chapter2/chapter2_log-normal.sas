...
title "Log Normal distribution on positive-cost data";
proc nlmixed data=&data;
	...
	*Method 1 for loglik;
	z = (LOG(TOTEXP17) - mu) / sigma;
    	...
    	*Method 2 for loglik;
    	*loglik = -log(TOTEXP17)-.5*log(2*constant("pi"))-log(sigma)-(1/(2*sigma**2))*(log(TOTEXP17)-mu)**2;
...
*3.D. Cross-validation process for Log Normal (5-fold validation);
%MACRO lognormal_model(fold,pred);
    ...
    title "Log Normal distribution on positive-cost data (training data excluding fold &fold.)";
...
/* Log Normal Model results: 
   ...
   Mean cross-validation absolute residual: 7014 */