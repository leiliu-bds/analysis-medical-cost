...
title "Log-skew-normal distribution distribution on positive-cost data";
proc nlmixed  data=&data;

	*Define initial values;
  	parms 	b0=6.8 b1_hibp=0.15 b1_chd=0.3 b1_strk=0.2
  			b1_emph=0.1 b1_chbron=0.1 b1_chol=0.2 b1_cancer=0.3
  			b1_diab=0.6 b1_jtpain=0.3 b1_arth=0.3 b1_asth=0.3
  		  	b2=-.25  b3_hispanic=1 b3_black=0 b3_other=0	
  		  	b4_18=-.5 b4_25=-.4 b4_35=-.3
  		  	b4_45=-.2	b4_65=0.1 b4_75=0.17
  		  	b5_northeast=0.3 b5_midwest=0.1 b5_west=0.1
  		  	b6=2
	...
	*Output expected value;
	predict E_y out=logskew;

	*Marginal Effects;
	ESTIMATE "marginal effect-hibp" 		2*exp(b0+b1_hibp+(exp((d0 + d1_hibp)/2))**2/2)*CDF('NORMAL',lambda*exp((d0 + d1_hibp)/2)/sqrt(exp((d0 + d1_hibp)/2)**2 + lambda**2))-2*exp(b0+(exp(d0/2))**2/2)*CDF('NORMAL',lambda*exp(d0/2)/sqrt(exp(d0/2)**2 + lambda**2));
	...
	ESTIMATE "marginal effect-jtpain" 		2*exp(b0+b1_jtpain+(exp((d0 + d1_jtpain)/2))**2/2)*CDF('NORMAL',lambda*exp((d0 + d1_jtpain)/2)/sqrt(exp((d0 + d1_jtpain)/2)**2 + lambda**2))-2*exp(b0+(exp(d0/2))**2/2)*CDF('NORMAL',lambda*exp(d0/2)/sqrt(exp(d0/2)**2 + lambda**2));
	...
	*Partial Elasticity;
	ESTIMATE 'partial elasticity-HIBP' 		log(2*exp(b0+b1_hibp+(exp((d0 + d1_hibp)/2))**2/2)*CDF('NORMAL',lambda*exp((d0 + d1_hibp)/2)/sqrt(exp((d0 + d1_hibp)/2)**2 + lambda**2)))-log(2*exp(b0+(exp(d0/2))**2/2)*CDF('NORMAL',lambda*exp(d0/2)/sqrt(exp(d0/2)**2 + lambda**2)));
...
data logskew_residual;
	set logskew;
	resid = TOTEXP17-pred;					
	abs = abs(resid);						
run;

proc univariate data=logskew_residual;
	var resid abs;
run;

/* Log-Skew-Normal Model results: 
   Mean residual: -1140	
   Mean absolute residual: 6995	 */
...
*3.D. Cross-validation process for Log-Skew-Normal (5-fold validation);
%MACRO logskewnormal_model(fold,pred);
    ...
    title "Log-Skew-Normal distribution on positive-cost data (training data excluding fold &fold.)";
	...
	*Store the parameters;
	...
	%parameter(name=lambda);
    	...
    	* Make predictions using the fitted model;
		...
		value1 = sigma**2 + &lambda**2;
		value4=&lambda*sigma/sqrt(value1);
		pred=2*exp(mu+sigma**2/2)*CDF('NORMAL',value4);
...
/* Log-Skew-Normal Model results: 
   Mean cross-validation residual: -1140
   Mean cross-validation absolute residual: 7013 */